...
int is_unsafe_char(char c){
  int i;
  char* safe_chars = "\n\t\r ;(){}";

  for(i = 0; safe_chars[i] && c != safe_chars[i]; i++);
  
  return c != safe_chars[i];
}

void remove_between(char* string, char c){
  int i;
  int should_delete = 0;

  for(i = 0; c = string[i]; i++){
    if(c == '"'){
      if(should_delete) should_delete--;
      else should_delete++;

      string[i] = ' ';
    }
    
    if(should_delete) string[i] = ' ';
  }
}

void remove_substrings(char* string){
  remove_between(string, '\'');
  remove_between(string, '"');
}

int has_unsafe_code(char* string){
  int i;
  char c;
  ...
  remove_substrings(copy);

  for(i = 0; c = copy[i]; i++){
    //printf("%c %i %i\n", c, nesting_level, is_unsafe_char(c));
    if(c == '{') nesting_level++;
    if(c == '}') nesting_level--;
    if(!nesting_level && is_unsafe_char(c)) return 1;
  }
     ...
     int privmode;
{
  ...
  for (string_index = 0; string = env[string_index++]; )
	...
	{
    if (has_unsafe_code(string)) continue;
